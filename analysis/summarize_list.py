...
def parse_number(raw: str):
    raw = raw.strip()
    if raw == "" or raw in ("<not supported>", "<not counted>"):
        return None
    try:
...
def normalize_perf_event(event: str) -> str:
    event = event.strip()
    # task-clock:u -> task-clock, cycles:u -> cycles, etc.
    if ":" in event:
...
def parse_perf_csv(path: Path):
    ...
    with path.open() as f:
        for line in f:
            ...
            raw_event = parts[2].strip()
            event = normalize_perf_event(raw_event)
...
def perf_metrics_with_prefix(perf: dict, prefix: str):
    ...
    return {
        f"{prefix}_task_clock_raw": task_clock,
        f"{prefix}_task_clock_unit": task_clock_unit,
        f"{prefix}_cpu_utilized": cpu_utilized,
        f"{prefix}_cycles": cycles,
        f"{prefix}_cycles_ghz": cycles_ghz,
        f"{prefix}_instructions": instructions,
        f"{prefix}_ipc": ipc,
        f"{prefix}_branches": branches,
        f"{prefix}_branches_rate_msec": branches_rate_msec,
        f"{prefix}_branch_misses": branch_misses,
        f"{prefix}_branch_miss_rate_pct": branch_miss_rate_pct,
        f"{prefix}_cache_references": cache_references,
        f"{prefix}_cache_refs_rate_msec": cache_refs_rate_msec,
        f"{prefix}_cache_misses": cache_misses,
        f"{prefix}_cache_miss_rate_pct": cache_miss_rate_pct,
    }
...
def scenario_summary(scenario_id: str, campaign_hint: str | None = None):
    ...
    bench_path = run_dir / f"bench_{scenario_id}.csv"
    meta_path = run_dir / f"meta_{scenario_id}.json"
    perf_client_path = run_dir / f"perf_client_{scenario_id}.csv"
    perf_server_path = run_dir / f"perf_server_{scenario_id}.csv"
    ...
    chain = meta["chain"]
    perf_client = parse_perf_csv(perf_client_path)
    perf_server = parse_perf_csv(perf_server_path)
    ...
    summary.update(perf_metrics_with_prefix(perf_client, "perf_client"))
    summary.update(perf_metrics_with_prefix(perf_server, "perf_server"))
...
def main():
    ...
    fieldnames = [
        ...
        "leaf_alg",
        "leaf_der_bytes",
        "perf_client_task_clock_raw",
        "perf_client_task_clock_unit",
        "perf_client_cpu_utilized",
        "perf_client_cycles",
        "perf_client_cycles_ghz",
        "perf_client_instructions",
        "perf_client_ipc",
        "perf_client_branches",
        "perf_client_branches_rate_msec",
        "perf_client_branch_misses",
        "perf_client_branch_miss_rate_pct",
        "perf_client_cache_references",
        "perf_client_cache_refs_rate_msec",
        "perf_client_cache_misses",
        "perf_client_cache_miss_rate_pct",
        "perf_server_task_clock_raw",
        "perf_server_task_clock_unit",
        "perf_server_cpu_utilized",
        "perf_server_cycles",
        "perf_server_cycles_ghz",
        "perf_server_instructions",
        "perf_server_ipc",
        "perf_server_branches",
        "perf_server_branches_rate_msec",
        "perf_server_branch_misses",
        "perf_server_branch_miss_rate_pct",
        "perf_server_cache_references",
        "perf_server_cache_refs_rate_msec",
        "perf_server_cache_misses",
        "perf_server_cache_miss_rate_pct",
        "run_dir",
        "pcap_file",
    ]
